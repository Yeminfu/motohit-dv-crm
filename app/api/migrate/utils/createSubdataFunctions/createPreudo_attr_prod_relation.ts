...
export default async function createPreudo_attr_prod_relation() {
  ...
  for (
    ...
    for (
      ...
      attributeIndex < attributesByCategory.length;
      attributeIndex++
    ) {
      ...
      for (
        ...
        for (
          let attrprodvalindex = 0;
          attrprodvalindex < getAttributeValues.length;
          attrprodvalindex++
        ) {
...
async function createattr_prod_relation(
...
async function getAttributesValues(
  idCategory: number
...
async function getProductsByategory(
  idCategory: number
): Promise<{ id: number }[]> {
  return await dbWorker(
    `
