                ...
                <table className="table table-striped">
                    ...
                    <tbody>
                        {products.map(product => <tr key={product.id}>
                            ...
                            <td>{product.name}</td>
                            <td>{product.code}</td>
                            <td><ReturnProductFromArchive idProduct={product.id} productName={product.name} /></td>
                        </tr>)}
                    </tbody>
                </table>
            </>
...
async function getProductsFromArchive(): Promise<ProductFromDB[]> {
    ...
    const products = await connection.query(`select * from ${process.env.TABLE_PREFIX}_products where isArchived = 1`)
        .then(([x]: any) => x)
        .catch(err => {
            console.log('err #fj48', err);
            return [];
        })
